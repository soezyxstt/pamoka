...
All uploads go through **R2** only. There is no local-blob or alternate object-storage write path.

- Next.js client helper: `src/lib/r2-media.ts`; Next.js server module and route: `src/server/media/r2.ts` and `src/app/api/media/upload/route.ts`.
- Laravel uses the same prepare, direct PUT, and complete contract at `/api/media/upload/prepare` and `/api/media/upload/complete`.
- Required configuration is `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_PUBLIC_URL`, and optional `R2_REGION=auto`.
...
| 006 | Add the media library with authenticated R2 uploads | DONE |
...
| 013 | Migrasi total Next.js ke Laravel + Inertia React | IN PROGRESS (Stage 10I) |
...
- Plan 013 Stage 4 core data foundation is complete in `laravel/`: local MySQL configuration, isolated `pamoka_test` PHPUnit database, UUID models and migrations for organization periods, editions, categories, selection stages, and participants, plus the fixed `CategoryCode` enum. The Laravel schema was applied to the explicitly authorized Aiven `defaultdb` target on 2026-09-14; operator content import and public cutover have not happened.
...
- Plan 013 Stage 9K edition and category authoring is complete in `laravel/`: edition listing, draft creation, fixed `JD`, `MD`, `JR`, and `MR` category codes, year and slug validation, activation reason, archiving of the previous active edition, permission checks, transactional writes, audit records, and an Inertia page are available under `/admin/content/editions`. External R2 handshake, operator database import, public cutover, and authenticated browser visual QA remain outside this slice.
- Plan 013 Stage 10H R2 storage reconciliation is implemented in both applications: the Next.js `/api/media/upload` route and Laravel `/api/media/upload/prepare` plus `/api/media/upload/complete` endpoints use finite image, video, and PDF policy, `media.manage` authorization, presigned PUT URLs, server-side object `HEAD` verification, idempotent `media_assets` persistence, and transactional audit. No local blob or alternate object-storage write path exists. External R2 handshake, bucket CORS and delivery-domain setup, legacy object migration, operator database import, public cutover, and authenticated browser visual QA remain outside this slice.
- Plan 013 Stage 10I Vercel Hobby target is configured in `laravel/`: `Dockerfile.vercel` builds Composer dependencies and Vite assets in separate stages, FrankenPHP serves Laravel through the public front controller, `.dockerignore` excludes environment secrets and local dependencies, compiled views use `/tmp`, logs use stderr, and production session or cache defaults use database-backed storage. The container build and production deployment completed. Production route smoke verifies home, about, event, news, finalist and semifinalist lists or details, voting, results, and legacy pages. When the Aiven schema has no imported content, public read models now use the committed 2025 snapshot fallback, including 44 finalists, 16 semifinalists, participant detail media, and the historical voting campaign with local QR assets. Aiven TLS or schema access from Vercel is exercised by the successful requests; R2 handshake, operator content import, and authenticated browser QA remain unverified.
- Plan 013 Stage 10A complete in `laravel/`: `moka:rehearse-2025` validates the participant, news, public media, organization, and voting fixtures before writing, runs those importers within one outer transaction, is idempotent, and permits apply only on the local `pamoka_test` database. The page-section fixture and importer are included in Stage 10D. Focused route checks cover home, about, event, news detail, finalists, and voting results after rehearsal. Operator import, editor parity, external provider handshake, authenticated browser QA, and public cutover remain outside this slice.
...
- Plan 013 Stage 10D complete for the public asset and page-section foundation in `laravel/`: active-edition `ready` site asset bindings are read through the fixed manifest with MIME validation and local fallback, and `Home` or `About` consume the bound visual assets. The additive `page_sections` model and migration, published active-edition reader for `tentang.misi` or `tentang.video`, 2025 fixture, and idempotent import command are included in the complete rehearsal. The full Laravel suite passed with 116 tests and 1,560 assertions, alongside Pint, TypeScript, Vite build, and diff checks. Route or data parity, unresolved 2025 title or social mapping, operator import, external provider handshake, authenticated browser QA, and public cutover remain outside this slice.
- Plan 013 Stage 10E complete for public edition identity in `laravel/`: `PublicEditionCatalog` reads the active edition slogan and active programs with a safe fallback, the 2025 edition-settings fixture and idempotent importer are included in the full rehearsal, and Home consumes the edition-owned values. Focused import or home tests and full rehearsal passed; the suite or static gates are recorded after the final checkpoint. Route or data parity, unresolved 2025 title or social mapping, operator import, external provider handshake, authenticated browser QA, and public cutover remain outside this slice.
- Plan 013 Stage 10F complete for legacy route compatibility in `laravel/`: `/z_contact___`, `/z_pasanggiri__`, and `/z_pasanggiri__/voting/{name}` now render Inertia pages on the original paths. The Pasanggiri page reads the rehearsed 44 finalists and 68 sponsors through Laravel read models, while the spotlight page keeps a safe fallback for unknown slugs. The full suite passed with 122 tests and 1,622 assertions, alongside Pint, TypeScript, Vite build, and diff checks. No Next.js source or operator database was changed.
- Plan 013 Stage 10G complete for public route parity in `laravel/`: the rehearsal-backed matrix covers home, tentang, three news details, six events, four categories across finalist, semifinalist, voting, and result routes, plus all three legacy compatibility paths. Every checked URL returned HTTP 200 with the expected Inertia component. The focused matrix passed with 337 assertions; the full suite or static gates are recorded after the final checkpoint.
...
- Plan 013 current checkpoint is Stage 10I. The remaining deployment gates are R2 handshake, authenticated browser QA, operator content import, unresolved 2025 title or social mapping, and public cutover. Production public route smoke, Aiven-backed request execution, and snapshot fallback content are verified; the authorized schema migration is complete.
- Plan 013 Stage 10C passed with 8 focused authoring tests and 75 assertions. The full Laravel suite at that checkpoint passed with 112 tests and 1,508 assertions, Pint, Laravel TypeScript check, Vite production build, and the rich text studio contracts on isolated MySQL `pamoka_test`. Stage 10D now adds the public asset or page-section foundation and raises the suite to 116 tests and 1,560 assertions. The rehearsal and editor data remain local only; final route or data parity, 2025 title or social mapping, external provider handshake, authenticated browser QA, and public cutover are still open.
- Plan 013 Stage 10D adds the public binding and page-section foundation. Only ready media from the active edition and manifest-approved MIME types reach public props; unpublished or other-edition page sections are excluded. The 2025 rehearsal now includes the page-section fixture and remains local only. Final route or data parity, 2025 title or social mapping, external provider handshake, authenticated browser QA, and public cutover are still open.
- Plan 013 Stage 10E adds the active-edition slogan or program read model and the edition-settings fixture. The complete rehearsal remains local only and now verifies six program records without duplication. Final route or data parity, 2025 title or social mapping, provider handshake, operator import, authenticated browser QA, and public cutover are still open.
- Plan 013 Stage 10F adds compatibility pages for the three source legacy routes. They use the Laravel public participant and sponsor read models, preserve unknown spotlight slugs without unsafe redirects, and remain sidecar pages until public cutover. Final route or data parity, 2025 title or social mapping, provider handshake, operator import, authenticated browser QA, and public cutover are still open.
- Plan 013 Stage 10G adds the complete public route parity matrix. The matrix runs after the local 2025 rehearsal and checks expected Inertia components across every canonical public family, all six event slugs, all four participant categories, news details, and the three legacy paths. Final route or data parity, 2025 title or social mapping, provider handshake, operator import, authenticated browser QA, and public cutover are still open.
- Plan 013 migration is staged and checkpointed. Stage 1 through Stage 9K passed their focused Laravel contracts, isolated MySQL rehearsal gates, Pint, Laravel TypeScript check, and Vite production build. Stage 10A passed the complete 2025 rehearsal gate. Stage 10B passed the rich text authoring gate, Stage 10C passed the autosave or preview gate, Stage 10D passed the public binding or page-section foundation, Stage 10E passed edition identity and program parity, Stage 10F passed legacy route compatibility, and Stage 10G passed the public route matrix with 337 assertions. Stage 10H has the R2 storage boundary and local verification. Stage 10I adds the Vercel Hobby container target, production deployment, and committed public snapshot fallback. The authorized Laravel schema migration is applied to Aiven `defaultdb`; public route smoke and Aiven-backed request execution are verified, while external provider handshakes, operator content import, authenticated browser QA, and public cutover remain explicit reconciliation items.
...
- **Media**: R2 (`src/server/media/r2.ts`, `/api/media/upload`), policy in `src/server/media/policy.ts`.
