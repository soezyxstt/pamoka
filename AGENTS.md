...
| 013 | Migrasi total Next.js ke Laravel + Inertia React | IN PROGRESS (Stage 10A) |
...
- Plan 013 current checkpoint is Stage 10A. The next implementation slice is final route or data parity gates and editor parity; 2025 title or social mapping, external provider handshake, operator import, and public cutover remain explicit reconciliation items.
- Plan 013 Stage 10A passed with 3 focused tests and 114 assertions. The full Laravel suite passed with 110 tests and 1,475 assertions, Pint, Laravel TypeScript check, Vite production build, and a twice-applied rehearsal on isolated MySQL `pamoka_test`. The rehearsal remains local only; operator import, editor parity, external provider handshake, authenticated browser QA, and public cutover are still open.
- Plan 013 migration is staged and checkpointed. Stage 1 passed Laravel feature tests, TypeScript, Vite production build, and local HTTP verification. Stage 2 inventory is complete. Stage 3 passed 5 feature tests, TypeScript, Vite production build, 10 route inspection, HTTP smoke, Inertia response, 404 response, and desktop or narrow viewport checks. Stage 4 passed 7 feature tests with 172 assertions, Pint, TypeScript, Vite production build, local MySQL migration, and foreign key or unique key inspection. Stage 5 foundation passed 20 feature tests with 263 assertions, Pint, TypeScript, Vite production build, auth migration and seed, HTTP smoke, and login visual QA. Stage 6 passed 22 feature tests with 289 assertions, Pint, TypeScript, Vite production build, public asset and HTTP smoke, and desktop or narrow viewport QA. Stage 7A passed 28 feature tests with 298 assertions, Pint, TypeScript, Vite production build, participant route HTTP smoke, and desktop or narrow viewport QA. Stage 7B passed 29 feature tests with 316 assertions, Pint, TypeScript, Vite production build, local profile migration, and public profile prop coverage. Stage 7C passed 32 feature tests with 379 assertions, Pint, Laravel TypeScript check, Vite production build, root TypeScript check, exporter lint, stage-entry migration, nonlocal-target guard, and idempotent import rehearsal on `pamoka_test`. Stage 8A passed 5 focused public route tests with 81 assertions, Pint, Laravel TypeScript check, Vite production build, local asset checks, and Inertia HTTP smoke for the event route. Stage 8B passed 2 focused home tests with 43 assertions, full Laravel tests at 36 tests and 415 assertions, Pint, Laravel TypeScript check, and Vite production build. Stage 8C passed 3 focused importer tests with 37 assertions, full Laravel tests at 39 tests and 452 assertions, Pint, Laravel TypeScript check, Vite production build, dry-run validation, and idempotent rehearsal on `pamoka_test`. Stage 8D passed 4 focused tests with 59 assertions, full Laravel tests at 43 tests and 511 assertions, Pint, Laravel TypeScript check, Vite production build, migration, dry-run validation, gallery-item constraint, and idempotent rehearsal on `pamoka_test`. Stage 8E passed 6 focused tests with 85 assertions, full Laravel tests at 49 tests and 596 assertions, Pint, Laravel TypeScript check, Vite production build, migration, dry-run validation, nonlocal-target guards, and idempotent organization rehearsal on `pamoka_test`. Stage 8F passed 9 focused tests with 150 assertions, full Laravel tests at 56 tests and 696 assertions, Pint, Laravel TypeScript check, Vite production build, migration, dry-run validation, nonlocal-target guards, idempotent voting rehearsal, result visibility checks, monitor permission checks, and HTTP smoke on `pamoka_test`. Stage 9A passed 6 focused tests with 53 assertions, full Laravel tests at 62 tests and 749 assertions, Pint, Laravel TypeScript check, Vite production build, local migration and authoring route inspection on `pamoka_test`. Stage 9B passed 4 focused tests with 53 assertions, full Laravel tests at 66 tests and 802 assertions, Pint, Laravel TypeScript check, Vite production build, local media authoring migration and 33 route inspection on `pamoka_test`. Stage 9E passed 4 focused tests with 66 assertions, full Laravel tests at 79 tests and 1.001 assertions, Pint, Laravel TypeScript check, Vite production build, migration, and organization route inspection on `pamoka_test`. The next slice is panitia, edition settings, site assets, voting operations, and media library authoring; 2025 title mapping, media upload, full editor parity, and public cutover remain explicit reconciliation items.
