...
| 013 | Migrasi total Next.js ke Laravel + Inertia React | IN PROGRESS (Stage 8C) |
...
- Plan 013 Stage 8A event route slice is complete in `laravel/`: the six hardcoded 2025 event entries now render through an Inertia React page with ten local WebP images per event and the preserved 68 sponsor logos. Invalid event slugs return 404. CMS event data, operator database migration, news, and public cutover remain outside this slice.
