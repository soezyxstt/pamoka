...
| **013** | Migrasi total Next.js ke Laravel + Inertia React | [`013-route-domain-inventory.md`](013-route-domain-inventory.md) dan `laravel/` | **IN PROGRESS (Stage 8B)** | Fondasi Laravel, data inti MySQL, autentikasi admin, RBAC, approval akses, audit, konteks edisi aktif, public core, profil peserta, route acara, serta snapshot berita sudah tersedia sebagai sidecar; Next.js lama tetap tidak diubah sebagai sumber pembanding sampai setiap checkpoint lolos. |
...
- **Status**: IN PROGRESS. Stage 7A, 7B, rehearsal Stage 7C, Stage 8A, dan Stage 8B selesai; pemetaan gelar masih menunggu source atau input operator. Tahap berikutnya adalah fondasi model berita atau route galeri dan sponsor berbasis data Laravel. Admin CMS penuh, handshake OAuth nyata, dan runtime publik belum menjadi target cutover.
