...
| **013** | Migrasi total Next.js ke Laravel + Inertia React | [`013-route-domain-inventory.md`](013-route-domain-inventory.md) dan `laravel/` | **IN PROGRESS (Stage 8E)** | Fondasi Laravel, data inti MySQL, autentikasi admin, RBAC, approval akses, audit, konteks edisi aktif, public core, profil peserta, acara, galeri, sponsor, berita detail, dan organisasi publik sudah tersedia sebagai sidecar; Next.js lama tetap tidak diubah sebagai sumber pembanding sampai setiap checkpoint lolos. |
...
- **Status**: IN PROGRESS. Stage 7A, 7B, rehearsal Stage 7C, Stage 8A, Stage 8B, Stage 8C, Stage 8D, dan Stage 8E selesai; pemetaan gelar masih menunggu source atau input operator. Tahap berikutnya adalah read model dan import voting, monitor, serta admin authoring bertahap. Admin CMS penuh, handshake OAuth nyata, dan runtime publik belum menjadi target cutover.
